...
/**
 * Horizontal bars for a small categorical breakdown. Monochrome by design:
 * every bar is labelled directly with its value, so no legend or colour
 * coding is needed.
 */
    ...
    <div className="stack" style={{ gap: 10 }} role="list">
      {data.map((d, i) => (
        <div key={d.label} role="listitem" aria-label={`${d.label}: ${d.value}`} style={{ display: 'grid', gridTemplateColumns: 'minmax(120px, 34%) 1fr auto', gap: 12, alignItems: 'center' }}>
          ...
          <span style={{ height: 14, background: 'var(--surface-2)', border: 'var(--border)', position: 'relative', overflow: 'hidden' }}>
            <motion.span
              style={{ position: 'absolute', inset: 0, background: 'var(--ink)', transformOrigin: 'left' }}
              initial={{ scaleX: 0 }}
